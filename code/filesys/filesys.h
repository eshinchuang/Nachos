...
class FileSystem {
  public:
    ...
    FileSystem() {
        for (int i = 0; i < 10; i++) filePtr[i] = NULL;
    }
	...
	*/
	...
	//you need to use this filePtr to manage the current file
	...
	int WriteFile1(char *buffer, int size)
	{ 
            return filePtr[0]->Write(buffer,size);
        
    }

    int ReadFile(char *buffer, int size){
      
            return filePtr[0]->Read(buffer,size);
